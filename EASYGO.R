#################################################
## This is EASYGO, the Easy Gene Ontology tool ##
...
# The file is stored in the same folder where EASYGO.R is located
...
# (2) A heatmap EASYGO.*.GO_Landscape.pdf containing the stepwise thresholding 
# (3) A heatmap EASYGO.*.Gene_Landscape.pdf connecting
# the most relevant DE genes with the most likley associated GO terms
# (4) A heatmap EASYGO.*.complete.pdf containing both (2) and (3)
...
for(filename in c('DEGENES.ALDO-RET.tab')) 
{
...
pdf(paste('./easygo_results/EASYGO.',filename,'.',workname,'.complete.pdf', sep=''),
    width=unit(wht0+wht1+whta+wht2+2+1,'inch'));
draw(ht1+hta+ha+hs+hz+ht0+ht2,
    heatmap_legend_side = "left",
    column_title_gp = gpar(fontsize = 10, fontface = "bold"),
    column_title=paste('EASYGO results of "',filename,'" : ',filelabel,sep='')
    );
dev.off()

pdf(paste('./easygo_results/EASYGO.',filename,'.',workname,'.Gene_Landscape.pdf', sep=''),
	width=unit(wht0+wht2+whta+2,'inch'));
    draw(ha+ht0+hta+ht2,
    column_title_gp = gpar(fontsize = 10, fontface = "bold"),heatmap_legend_side = "left",
    column_title=paste('EASYGO results of "',filename,'"',sep='')
    );
dev.off()

pdf(paste('./easygo_results/EASYGO.',filename,'.',workname,'.GO_Landscape.pdf', sep=''),
